...
public class serviceLayer implements serviceLayerInterface {
	...
	public String login(String request, ConnectionHandler handler) {
		...
		if (requests[0].equals("LOGIN") && requests.length == 3) {
			...
			if (gameCenter.login(requests[1], requests[2], handler)) {
				...
				return "LOGIN DONE " + user.getID() + " " + user.getName() + " " + user.getTotalCash() + " "
						+ user.getScore() + " " + user.getLeague() + " " + user.getAvatar();
	 ...
	 * 
	 * @param request
	 *            is string that has this format: "EDITUSERAVATAR *USER NAME*
	 *            *NEW AVATAR*"
	 * @return "EDITUSERAVATAR DONE" if succeed to edit the user password,
	 *         "EDITUSERAVATAR FAILED" else
	 */
	@Override
	...
	private String gamesToIDs(LinkedList<Game> games) {
		...
		for (Game game : games) {
			...
			gameDetails = gameDetails + "GAMEID=" + game.getGameID() + " BUYIN="
					+ game.getpreferences().getBuyInPolicy() + " SETS="
					+ (game.getpreferences().getMaxPlayersNum() - game.getPlayerNumber()) + " ENDGAME ";
	...
	private GamePreferences parseGamePrefs(String gamePrefs) {
		...
		int potLimit = -1, buyInPolicy = 0, chipPolicy = 0, minBet = 0, minPlayersNum = 2, maxPlayersNum = 8,
				league = -1;
		boolean spectatable = true, leaguable = false;

		// PREF = gameTypePolicy=*GAME TYPE POLICY*&potLimit=*POT
		// LIMIT*&buyInPolicy=*BUY IN POLICY*&chipPolicy=*CHIP POLICY*&
		// minBet=*MIN BET*&minPlayersNum=*MIN PLAY NUM*&maxPlayersNum=*MAX
		// PLAYER NUMBER*&
		...
		for (String pref : prefs) {
			...
			if (prefParts[0].equals("gameTypePolicy")) {
				if (prefParts[1].equals("NO_LIMIT")) {
					...
					gameTypePolicy = GameType.POT_LIMIT;
		...
		try {
			GamePreferences result = new GamePreferences(gameTypePolicy, potLimit, buyInPolicy, chipPolicy, minBet,
					minPlayersNum, maxPlayersNum, spectatable, leaguable, league);
			return result;
		} catch (Exception e) {
			return null;
		}
	...
	public String joinGame(String request) {
		...
		if (requests[0].equals("JOINGAME") && requests.length == 3) {
			if (gameCenter.joinGame(requests[1], requests[2])) {
				...
				return "JOINGAME " + requests[1] + " " + requests[2] + " DONE "
						+ GameToString((Game) gameCenter.getGameByID(requests[1]));
	...
	private String GameToString(Game game) {
		...
		result = result + "&CurrentPlayer="
				+ (game.getCurrentPlayer() != null ? game.getCurrentPlayer().getUser().getID() : "");
		result = result + "&table=";
	...
	public String spectateGame(String request) {
		...
		if (requests[0].equals("SPECTATEGAME") && requests.length == 3) {
			if (gameCenter.spectateGame(requests[2], requests[1])) {
				...
				return "SPECTATEGAME " + requests[1] + " " + requests[2] + " DONE "
						+ GameToString((Game) gameCenter.getGameByID(requests[1]));
	...
	/**
	 * @param action
	 *            is string that has this format: "CHATMSG *GameID* *UserID*
	 *            *MSG*"
	 */
	 ...
	 * 
	 * @param msg
	 *            is string that has this format: "WHISPERMSG *GameID* *UserID*
	 *            *reciverID* *MSG*"
	 */
	...
	public String getLeadersboard(String request) {
		String[] parts = request.split(" ");
		if (parts[0].equals("LEADERBOARD") && parts.length == 2) {
			LeaderboardType type = "CASH".equals(parts[1]) ? LeaderboardType.TotalGrossProfit
					: ("WIN".equals(parts[1]) ? LeaderboardType.HighestCashGain : LeaderboardType.GamesPlayedNumber);

			String leaders = gameCenter.getLeaderboard(type);
			if (leaders != null)
				return leaders;
		}
		return "Bad Request";
	...
	public String getUserStats(String request) {
		String[] parts = request.split(" ");
		if (parts[0].equals("STATS") && parts.length == 3) {
			int type = "WIN".equals(parts[1]) ? 1 : ("GROSS".equals(parts[1]) ? 2 : 0);
			if(type != 0)
				return gameCenter.getUserStats(type, parts[2]);
		}
		return "Bad Request";
	}
}
